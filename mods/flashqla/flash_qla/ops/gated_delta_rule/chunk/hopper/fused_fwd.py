...
def tilelang_fused_chunk_gdr_fwd(
    ...
    is_varlen,
    is_cp,
    block_DV=32,  # [BLACKWELL] was 128; reduced to fit GB10's 99 KiB shmem
    # [BLACKWELL SINGLE-BUFFER] (was double-buffered; collapsed to fit shmem)
    num_stages=2,  # [BLACKWELL] kept at 2 - num_stages=1 hits a TileLang indexing bug
):
    ...
    def tilelang_fused_chunk_gdr_fwd_kernel(
        ...
        with T.Kernel(T.ceildiv(DV, block_DV) * batch_size * H, threads=512) as (bbhv,):
            ...
            v_shared = T.alloc_shared((2, block_S, block_DV), dtype=qkva_dtype)  # [BLACKWELL HYBRID] double-buffered for producer/consumer overlap; was tried single — measured 1.06x vs 1.73x hybrid at block_DV=32
            a_shared = T.alloc_shared((2, block_S, block_S), dtype=qkva_dtype)  # [BLACKWELL HYBRID] double-buffered
            ...
            data_is_ready = T.alloc_barrier(arrive_count=96)
            data_is_free = T.alloc_barrier(arrive_count=384)
            ...
            if tx < 128:
                ...
                for i_s in T.serial(num_iters):
                    # [STAGE 0]
                    T.barrier_wait(data_is_ready, (i_s + 0) % 2)
                ...
                for i_s in T.serial(num_iters):
                    ...
                    # U = K @ S
                    T.gemm_v1(
                        k_shared[:, :], h_shared, u_fragment, clear_accum=True
                    )
                ...
                for i_s in T.serial(num_iters):
                    ...
                    # [STAGE 0] 0
                    T.barrier_wait(bar_0, i_s % 2)
                    # P = Q K^T  [BLACKWELL] gemm_v2 — gemm_v1 produces wrong
                    # result on SM_120/121 with our buffer config (verified
                    # via diagnostic_p.py — raw QK^T diffed by 0.45 vs ~0.29
                    # expected magnitude).
                    T.gemm_v2(
                        q_shared[:, :],
                        k_shared[:, :],
                    ...
                    for j_s, j_t in T.Parallel(block_S, block_S):
                        g_fragment[j_s, j_t] = (
                            g_shared[j_s] - g_shared[j_t]
                        )
                    for j_s, j_t in T.Parallel(block_S, block_S):
                        g_fragment[j_s, j_t] = T.if_then_else(
                            j_s >= j_t,
                            T.exp2(g_fragment[j_s, j_t] * 1.442695),
                            0.0,
                        )
                    # Ag = G * Ar * b
                    for j_s, j_t in T.Parallel(block_S, block_S):
                    ...
                    # O = Q @ S
                    T.gemm_v1(
                        q_shared[:, :], h_shared, o_fragment, clear_accum=True
                    )
                    ...
                    # O = s * g * O
                    for j_s, j_k in T.Parallel(block_S, block_DV):
                ...
                if tx < 384 + 32:
                    for i_s in T.serial(num_iters):
                        T.barrier_wait(data_is_free, (i_s + 1) % 2)
                        left = seq_start_idx + i_s * block_S
                        right = left + block_S

                        # Load Q
                        T.copy(
                            q[batch_idx, left:right, bhg, 0:DK], q_shared[:, :]
                        )
                        # Load K
                        T.copy(
                            k[batch_idx, left:right, bhg, 0:DK], k_shared[:, :]
                        )
...
def fused_gdr_fwd(
    ...
    # [BLACKWELL] GB10 has only 99 KiB opt-in shmem (vs Hopper's 228 KiB).
    # Kernel single-buffered (see [BLACKWELL SINGLE-BUFFER] in alloc_shared
    # block).  Budget: 49 KiB (DV-independent) + 768 B/block_DV.  Max
    # power-of-2 block_DV fitting in 99 KiB is 64 -> 49 + 48 = 97 KiB.
    grid_size = real_batch_size * H
    if torch.cuda.get_device_capability()[0] >= 10:
        # [BLACKWELL HYBRID] block_DV=32 with v/a double-buffered, rest
        # single-buffered.  Tried single-buffer block_DV=64 (1.06x) — hybrid
        # block_DV=32 wins (1.73x): pipelining v/a producer-consumer overlap
        # outweighs fewer kernel launches.
        block_DV = 32
