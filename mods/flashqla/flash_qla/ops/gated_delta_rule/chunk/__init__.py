...
# [BLACKWELL EXPERIMENT] gate bypassed — TileLang lowers the same DSL to
# whatever target the GPU advertises, so on SM_120/121 it should pick
# Blackwell-equivalent instructions instead of WGMMA.  Falls back to the
# original arch-named "hopper" submodule because the kernels themselves
# don't reference Hopper-only PTX directly.
_compute = tilelang.contrib.nvcc.get_target_compute_version()
print(f"[FlashQLA] bypassing arch gate; tilelang reports compute={_compute}")
...
def chunk_gated_delta_rule_fwd(
    ...
    if auto_cp:
        ...
        # [BLACKWELL] CP preprocessing requires the prepare_h kernel which
        # asks for >100 KiB shmem and won't fit on GB10.  The downstream
        # fused_gdr_fwd kernel allocates safe defaults when these are None.
        cp_seq_map = None
        raw_cu_seqlens = None
...
def chunk_gated_delta_rule(
    ...
    # [BLACKWELL FIX] upstream FlashQLA passes use_qk_l2norm_in_kernel here,
    # but ChunkGatedDeltaRuleFunction.forward doesn't take it (l2norm has
    # already been applied above when use_qk_l2norm_in_kernel=True).  Drop
    # the extra arg.
